...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="section-backdrop section-slant relative bg-[var(--bg)] px-4 pb-24 pt-32 sm:px-6">
      <div className="geo-shape hex-frame absolute right-6 top-16 hidden h-16 w-16 bg-gradient-to-br from-[var(--accent)]/20 to-[var(--accent-2)]/20 lg:block" aria-hidden="true" />
        ...
        <div className="mt-14 grid auto-rows-[150px] grid-cols-2 gap-4 sm:auto-rows-[160px] sm:grid-cols-4">
          {galleryImages.map((image, index) => (
            <motion.button
              ...
              className={`group relative block overflow-hidden rounded-2xl border border-[var(--border)] shadow-accent-sm ${SPANS[index % SPANS.length]}`}
            >
              <img
                src={image.src}
                alt={image.alt}
